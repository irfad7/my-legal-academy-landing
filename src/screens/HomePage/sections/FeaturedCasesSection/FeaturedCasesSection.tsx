              ...
              <div className="space-y-8 md:space-y-12 px-4 md:px-16 lg:px-20">
                ...
                <div className="space-y-6 md:space-y-8 [font-family:'Playfair_Display',serif] text-[16px] sm:text-[18px] md:text-[22px] lg:text-[24px] leading-relaxed max-w-3xl mx-auto">
                  {benefitsList.map((benefit, index) => (
                    <div key={index} className="flex items-start gap-4 md:gap-6 lg:gap-8 group hover:scale-[1.02] transition-all duration-300 hover:bg-gray-50/50 rounded-lg p-4 -m-4 cursor-pointer">
                      <div className="flex-shrink-0">
                        <span className="[font-family:'Playfair_Display',serif] font-light text-[#0c0c0c] text-[28px] sm:text-[32px] md:text-[40px] lg:text-[44px] leading-none transition-all duration-300 group-hover:text-[#0e823e] group-hover:scale-110">
                          {index + 1}.
                        </span>
                      </div>
                      <div className="flex-1 space-y-3">
                        <div className="font-bold text-[#0c0c0c] text-[18px] sm:text-[20px] md:text-[24px] lg:text-[26px] transition-all duration-300 group-hover:text-[#0e823e]">
                          {benefit.title}
                        </div>
                        <div className="text-[#0c0c0c] transition-all duration-300 group-hover:text-[#4d5256]">{benefit.description}</div>
                      </div>
                    </div>
                  ))}
                </div>
